...
{		
	...
	private Camera CameraLeft, CameraRight, CameraAudience = null;
	...
	new void Awake()
	{
		...
		for (int i = 0; i < cameras.Length; i++)
		{
			...
			if(cameras[i].name == "CameraAudience")
				CameraAudience = cameras[i];
		}
	...
	void UpdateCameras()
	{
		...
		distOffset = 0.5f + (LensOffsetRight * 0.5f);
		perspOffset = LensOffsetRight;
		ConfigureCamera(ref CameraAudience, distOffset, perspOffset, 0);
	...
	bool ConfigureCamera(ref Camera camera, float distOffset, float perspOffset, float eyePositionOffset)
	{
		...
		if (camera.GetComponent<OVRLensCorrection>()) {
