...
public class OVRDistortionCamera : OVRComponent
{
	...
	void DistortEye(bool rightEye, RenderTexture undistorted)
	{
		...
		var dualScreen = gameObject.GetComponent<DualScreen> ();
		...
		if (!CameraController.LensCorrection) {
			...
			if (dualScreen.isDualScreen)
					rect = new Rect (rightEye ? 1f : 0f, 0f, 1f, 1f);
		...
		float quarterWidth = 0.25f * Screen.width;
		if(dualScreen.isDualScreen)
			GL.Viewport(new Rect(rightEye ? quarterWidth : 0f, 0f, quarterWidth, Screen.height));
		else
